...
class Preprocess:
    ...
    def _drop_id_columns(self, dataset):
        cols_to_drop = []
        
        for col in dataset.columns:
            # Skip everything if it's a float (features are usually floats)
            if dataset[col].dtype == 'float64':
                continue
                
            if any(id_word in col.lower() for id_word in ['id', 'index', 'user', 'row']):
                # Only drop if it's also highly unique (to avoid dropping 'User_Age')
                if dataset[col].nunique() > len(dataset) * 0.9:
                    cols_to_drop.append(col)
                    continue

            # The "Range" check
            # Only apply this to integer columns
            if dataset[col].dtype == 'int64':
                sorted_vals = dataset[col].sort_values().reset_index(drop=True)
                if sorted_vals.equals(pd.Series(range(len(dataset)))) or \
                sorted_vals.equals(pd.Series(range(1, len(dataset)+1))):
                    cols_to_drop.append(col)
    ...
    def preprocess(self):
        ...
        # Dropping ID columns
        dataset = self._drop_id_columns(dataset)
